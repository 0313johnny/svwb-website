    ...
    <q-page class="q-pa-lg">
        <div class="compare-area q-mx-auto">
            ...
            <div class="row items-center q-gutter-sm q-mb-md">
                <q-file v-model="csvFile" outlined dark dense accept=".csv" label="上傳賽事 CSV" class="csv-input"
                    @update:model-value="onFileSelected">
                    <template #prepend><q-icon name="upload_file" /></template>
                </q-file>
                <q-select v-model="selectedVersionId" outlined dark dense
                    :options="ARCHETYPE_VERSIONS.map(v => ({ label: v.label, value: v.id }))" emit-value map-options
                    label="牌組版本" style="min-width:180px" />
                <q-btn v-if="Object.keys(decksByClass).length" flat no-caps dense color="grey-5" icon="clear" label="清除"
                    @click="reset" />
            </div>
                ...
                <template v-if="activeClass !== null">
                    ...
                    <div class="row items-center q-gutter-xs q-mb-sm">
                        <q-btn no-caps unelevated dense size="sm"
                            ...
                            :label="`全部 (${decksByClass[activeClass]?.length ?? 0})`" @click="activeArchetype = null" />
                        <q-btn v-for="arch in availableArchetypes" :key="arch.id" no-caps unelevated dense size="sm"
                            :color="activeArchetype === arch.id ? 'primary' : 'grey-8'"
                            :label="`${arch.name} (${archetypeCounts.get(arch.id) ?? 0})`"
                            @click="activeArchetype = arch.id" />
                        <q-btn v-if="(archetypeCounts.get(null) ?? 0) > 0" no-caps unelevated dense size="sm"
                    ...
                    <div class="row items-center q-gutter-sm q-mb-sm">
                        ...
                        <q-space />
                        <!-- 排序按鈕 -->
                        <q-btn-toggle v-model="sortBy" dense flat no-caps rounded toggle-color="primary" color="dark"
                            text-color="white" :options="[
                                { label: '費用', value: 'cost' },
                                { label: '攜帶率', value: 'usage' },
                            ]" />
                        <q-btn flat dense round :icon="sortOrder === 'asc' ? 'arrow_upward' : 'arrow_downward'"
                            color="grey-4" @click="sortOrder = sortOrder === 'asc' ? 'desc' : 'asc'" />
                    </div>
                    <!-- 圖表 + 左側卡片列表疊層 -->
                    <div class="chart-wrap" :style="{ height: chartHeight }">
                        <!-- ECharts（y 軸隱藏，左側留空給卡片列表） -->
                        <VChart ref="chartRef" :option="chartOption" style="width:100%;height:100%" autoresize
                            @rendered="syncOverlay" />
                        <!-- 卡片列表疊在左側，每列由 convertToPixel 精確定位 -->
                        <div class="card-overlay">
                            <CardRow v-for="(card, i) in chartData" :key="card.cardId" :cost="card.cost"
                                :name="card.name" :rarity="card.rarity" :banner-url="card.bannerUrl"
